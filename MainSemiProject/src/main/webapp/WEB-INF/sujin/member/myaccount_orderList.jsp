	...
	<div class="col-md-12 py-3 mx-auto">
		...
		<c:if test="${not empty requestScope.ovolist}">
			<c:forEach var="ovo" items="${requestScope.ovolist}" >
				...
				<div class="mt-4">${ovo.order_no}</div>
				<div class="mt-4">${ovo.orderdate}</div>
				<div colspan="3" class="col-md-6 text-left" style="font-size: 10pt;">
		 			<div><label>수령자성명 : </label>${ovo.delivery_name}</div>
		 			<div><label>수령자연락처 : </label>${ovo.delivery_mobile}</div>
		 			<div><label>수령자주소 : </label>${ovo.delivery_address}</div>
		 			<c:if test="${not empty ovo.delivery_comment}">
		 			  <div><label>수령자요청사항 : </label>${ovo.delivery_comment}</div>
		 			</c:if>
		 		</div>
				<div id="total_price" style="font-size: 12pt; font-weight: bold;"><div class="mt-4"><fmt:formatNumber value="${ovo.total_price}" pattern="###,###" />원</div></div>
